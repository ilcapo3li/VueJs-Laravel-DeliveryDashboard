<template>
  <card>
    <form>
      ...
      <div :style="{'text-align':isRTL?'right':'left'}">
        <span
          v-show="errors.has('category_en_name')"
          class="error"
      ...
      <div :style="{'text-align':isRTL?'right':'left'}">
        <span
          v-show="errors.has('category_description')"
          class="error"
        >*{{ errors.first('category_description') }}</span>
      </div>
      <base-input
        :label="$t('category_rank')"
        v-model="categories.category_rank"
        :value="categories.category_rank"
        :class="isRTL?'text-right':'text-left'"
      ...
      <div :style="{'text-align':isRTL?'right':'left'}">
        <span
          v-show="errors.has('category_rank')"
          class="error"
...
export default {
  ...
  methods: {
        ...
        if (result) {
          axios
            .post(apiUrls.saveCategories(), {
              ...
              category_rank: this.categories.category_rank,
              logo: this.photo_id
            })
            .then(response => {
    ...
    DeletePhoto(file, error, xhr) {
      axios
        .post(apiUrls.removePhoto(), { photo_id: this.photo_id })
        .then(response => {
